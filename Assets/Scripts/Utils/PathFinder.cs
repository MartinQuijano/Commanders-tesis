...
public class PathFinder
{
    ...
    public List<OverlayTile> FindPathBlockedByEnemyUnits(OverlayTile start, OverlayTile end, List<OverlayTile> searchableTiles)
    {
        List<OverlayTile> openList = new List<OverlayTile>();
        List<OverlayTile> closedList = new List<OverlayTile>();
        ...
        while (openList.Count > 0)
        {
            ...
            foreach (var neighbour in neighbourTiles)
            {
                if (neighbour.tileData.isTerrainBlocked || closedList.Contains(neighbour) || (neighbour.characterOnTile != null && neighbour != end && !neighbour.characterOnTile.isFromCurrentPlayingTeam))
                {
                    continue;
                }
